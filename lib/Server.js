...
var Server = function (options) {
    ...
    self.server.on('error', function (error) {
        self.logger.error('node-relp', 'RELP Server Error', { error: error.stack || error })
    })
    ...
    switch (message.command) {
        case 'syslog':
            ...
            if (socket.relpHandshakeComplete === false) {
                ...
                self.logger.error(
		    'node-relp',
                    'Connection #' + message.socketId + ' (' + self.getAddress(socket) + ') sent a syslog message before the handshake, disconnecting'
                )
        ...
        default:
            //TODO: emit?
            self.logger.error('node-relp', 'Connection #' + message.socketId + ' (' + self.getAddress(socket) + ') sent an unknown command: ' + message.command)
            return socket.destroy()
    }
}
...
Server.prototype._setupConnection = function (socket) {
    ...
    socket.parser.on('error', function (error) {
        self.logger.error('node-relp', 'Connection #' + socket.socketId + ' (' + self.getAddress(socket) + ') had a parse error, disconnecting.', { error: error.stack || error })
        socket.destroy()
    })
    ...
    socket.on('error', function (error) {
        self.logger.error('node-relp', 'Connection #' + socket.socketId + ' (' + self.getAddress(socket) + ') had an error.', { error: error.stack || error })
        socket.destroy()
    })
...
/**
 * Gets the address to log for a socket
 *
 * @param {net.Socket} socket
 */
Server.prototype.getAddress = function (socket) {
    var str = socket.remoteAddress
    if (socket.hasOwnProperty('proxyDetails')) {
        str = socket.proxyDetails.clientIp + ', ' + str
    }
    return str
}
