...
Server.prototype._setupConnection = function (socket) {
    ...
    socket.parser.on('error', function (error) {
        self.logger.error('Connection #', socket.socketId, '(' + self.getAddress(socket) + ')', 'had a parse error, disconnecting.', JSON.stringify(error))
        socket.destroy()
    })
    ...
    socket.on('error', function (error) {
        self.logger.error('Connection #', socket.socketId, '(' + self.getAddress(socket) + ')', 'had an error:', JSON.stringify(error))
        socket.destroy()
    })
