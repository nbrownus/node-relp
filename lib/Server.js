    ...
    switch (message.command) {
        case 'syslog':
            ...
            if (socket.relpHandshakeComplete === false) {
                //TODO: emit?
                self.logger.error(
                    'Connection id', message.socketId, '(' + socket.remoteAddress + ')', 'sent a syslog message before the handshake, disconnecting'
                )
        ...
        default:
            //TODO: emit?
            self.logger.error('Connection id', message.socketId, '(' + socket.remoteAddress + ')', 'sent an unknown command:', message.command)
            return socket.destroy()
    }
}
...
Server.prototype._setupConnection = function (socket) {
    ...
    socket.parser.on('error', function (error) {
        self.logger.error('Connection #', socket.socketId, '(' + socket.remoteAddress + ')', 'had a parse error, disconnecting.', error)
        socket.destroy()
    })
    ...
    socket.on('error', function (error) {
        self.logger.error('Connection #', socket.socketId, '(' + socket.remoteAddress + ')', 'had an error:', error)
        socket.destroy()
    })
